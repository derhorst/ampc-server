...
out_play_track:
      ...
      case MPD_API_ADD_ARTIST_ALBUM:
          ...
          p_charbuf = strdup(c->content);
          char tok[256];
          strncpy (tok, p_charbuf, sizeof(tok));
          int play;
          unsigned int q;
          char **list = tokenize(tok , ',', '^',  &q);

          /* search for exact album_artist + album in db */
          if(q > 2) {
            if(strcmp(list[1], "play")) {
              play = 0;
            } else {
              play = 1;
          ...
          mpd_search_add_tag_constraint(mpd.conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ALBUM_ARTIST, list[2]);
          mpd_search_add_tag_constraint(mpd.conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ALBUM, list[3]);
...
out_search:
        ...
        goto out_artist_albums;
                ...
                free(p_charbuf);
                p_charbuf = strdup(c->content);
                n = mpd_get_artist_albums(mpd.buf, get_arg1(p_charbuf));
    out_artist_albums:
                free(p_charbuf);
                break;
        case MPD_SEND_LIST_PLAYLISTS:
                p_charbuf = strdup(c->content);
                if(strcmp(p_charbuf, "MPD_SEND_LIST_PLAYLISTS"))
        goto out_list_playlists;
                free(p_charbuf);
                n = mpd_list_playlists(mpd.buf);
    out_list_playlists:
                free(p_charbuf);
                break;
        case MPD_SEND_LIST_PLAYLIST_META:
                p_charbuf = strdup(c->content);
                if(strcmp(strtok(p_charbuf, ","), "MPD_SEND_LIST_PLAYLIST_META"))
        goto list_playlist_meta;
                ...
                free(p_charbuf);
                p_charbuf = strdup(c->content);
                n = mpd_list_playlist_meta(mpd.buf, get_arg1(p_charbuf));
    list_playlist_meta:
                free(p_charbuf);
                break;
...
char* mpd_song_get_filename(struct mpd_song const *song)
{
    char *str = "";
...
int mpd_put_current_song(char *buffer)
{
    ...
    if(mpd_song_get_tag(song, MPD_TAG_TRACK, 0) != NULL)
    {
        ...
        cur += json_emit_quoted_str(cur, end - cur, mpd_song_get_tag(song, MPD_TAG_TRACK, 0));
    }
    ...
    if(mpd_song_get_tag(song, MPD_TAG_DATE, 0) != NULL) {
        cur += json_emit_raw_str(cur, end - cur, ",\"date\":");
        cur += json_emit_quoted_str(cur, end - cur, mpd_song_get_tag(song, MPD_TAG_DATE, 0));
    }

    cur += json_emit_raw_str(cur, end - cur, ",\"file\":");
    cur += json_emit_quoted_str(cur, end - cur, mpd_song_get_filename(song));
...
int mpd_put_queue(char *buffer, unsigned int offset)
{
    ...
    while((entity = mpd_recv_entity(mpd.conn)) != NULL) {
        ...
        if(mpd_entity_get_type(entity) == MPD_ENTITY_TYPE_SONG) {
            ...
            // if(mpd_song_get_tag(song, MPD_TAG_ORIGINAL_DATE, 0) != NULL)
            // {
            //     cur += json_emit_raw_str(cur, end - cur, ",\"date\":");
            //     cur += json_emit_quoted_str(cur, end - cur, mpd_song_get_tag(song, MPD_TAG_ORIGINAL_DATE, 0));
            // } else
            if(mpd_song_get_tag(song, MPD_TAG_DATE, 0) != NULL) {
                cur += json_emit_raw_str(cur, end - cur, ",\"date\":");
                cur += json_emit_quoted_str(cur, end - cur, mpd_song_get_tag(song, MPD_TAG_DATE, 0));
...
int mpd_get_all_meta(char *buffer) {
  ...
  const char *end = buffer + MAX_SIZE;
  struct mpd_entity *entity;
  if (!mpd_send_list_all_meta(mpd.conn, NULL))
      RETURN_ERROR_AND_RECOVER("mpd_send_list_all_meta");

  cur += json_emit_raw_str(cur, end  - cur, "{\"type\":\"all_meta\",\"data\":[ ");
...
int mpd_get_artist_albums(char *buffer, char *artist)
{
    ...
    // struct mpd_song *current_song = mpd_run_current_song(mpd.conn);

    mpd_search_db_songs(mpd.conn, true);
    mpd_search_add_tag_constraint(mpd.conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ALBUM_ARTIST, artist);
    mpd_search_add_tag_constraint(mpd.conn, MPD_OPERATOR_DEFAULT, MPD_TAG_TRACK, "1");
    mpd_search_commit(mpd.conn);

    cur += json_emit_raw_str(cur, end  - cur, "{\"type\":\"artist_albums\",\"data\":[ ");
...
int mpd_list_playlists(char *buffer) {
    ...
    while((playlist = mpd_recv_playlist(mpd.conn)) != NULL) {
      cur += json_emit_raw_str(cur, end - cur, "{\"path\":");
      cur += json_emit_quoted_str(cur, end - cur, mpd_playlist_get_path(playlist));
      cur += json_emit_raw_str(cur, end - cur, "},");
    }
    /* remove last ',' */
    cur--;
...
int mpd_list_playlist_meta(char *buffer, char *name) {
    ...
    cur += json_emit_raw_str(cur, end  - cur, "{\"type\":\"playlist\",\"name\":");
    ...
    while((entity = mpd_recv_entity(mpd.conn)) != NULL) {
        ...
        switch (mpd_entity_get_type(entity)) {
            ...
            case MPD_ENTITY_TYPE_SONG:
                song = mpd_entity_get_song(entity);
                cur += json_emit_raw_str(cur, end - cur, "{\"type\":\"song\",\"file\":");
